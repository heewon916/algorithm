...
- 참고하면 좋을 문제: [백준-1238.파티](https://github.com/heewon916/algorithm/tree/ae542bf19cac81ee6c751b8cb1c96b7f4d76a7e5/%EB%B0%B1%EC%A4%80/Gold/1238.%E2%80%85%ED%8C%8C%ED%8B%B0)
- 위의 코드처럼 작성해도 괜찮다. 꼭 아래를 따를 필요는 없음.
- 이유: 결국 시작점이 정해져 있고, 간선의 가중치는 다른데, 모든 정점까지의 최단거리를 구해야 하는 것 => dist[] 배열 하나 만들어서 -무한대/+무한대로 초기화해두고, PriorityQueue + Comparator 써서 더 짧은 거리면 거리 갱신하는 거임.
- 주의할 점: `v[i] = true;` 이 코드를 미리 하면 -> while문이 못 돌아감. while문 안에서 처리하는 게 국룰임 
