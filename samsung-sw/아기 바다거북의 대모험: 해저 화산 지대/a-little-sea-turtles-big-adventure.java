...
/**
풀고 난 후기: 
- 한 턴 안에서 어떤 순서로 이루어지는지 봐야한다... 
- 그리고 열기 전파, 연쇄반응, 화석화 간의 순서가 동기인지, 비동기인지 봐야한다. 
- 지금처럼 main은 최대한 함수화하는 게 좋다는 생각도 들었고, 객체를 클래스화하는 것도 깔끔하구나 싶었다.
- 여태까지는 hashmap, list 같은 자료구조로 어떻게 상태를 나타내지? 그런 생각을 많이 했는데 
- 오늘은 풀면서 자료구조를 정하는 것보다, 상태 변경이 언제 어떻게 발생하는지 체크하는 게 더 중요하다는 걸 느꼈다.
*/